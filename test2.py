...
# 把df裡面的資料全部改成文字類型
df = df.astype(str)
# 刪除'INVOICE_NO'和'ITEM_NO'皆相同的資料(只保留一筆)
df_unique = df.drop_duplicates(subset=['INVOICE_NO', 'PRODUCT_TYPE'])
# 把相同'INVOICE_NO'所購買的'ITEM_NO'合併在一個集合內
...
rules.to_csv('FP-Growth_output.csv', index=False)
...
print(f"FP-Growth spend {end - start} seconds")